...
// Render event details in right panel (matching graph.js format exactly)
function renderEventDetailsView(event) {
    ...
    // Build HTML string matching graph.js format exactly
    const detailsHTML = `
        <div style="margin-bottom: 1.5rem; padding-bottom: 1rem; border-bottom: 2px solid rgba(0, 0, 0, 0.05);">
            ...
            <div style="font-size: 0.85rem; color: #64748b; margin-top: 0.5rem;">
                <strong>Country:</strong> ${event.country || 'Unknown'}
            </div>
...
function updateFactionPanel(factionId, events, countries, factionNodeData) {
    ...
    // Create faction info section
    const factionSection = leftPanel.insert("div", ":first-child")
        .attr("class", "country-info-section")
        .style("display", "block");

    factionSection.append("h2")
    ...
    const region = factionNodeData?.region || "Multiple Regions";

    // Statistics
    factionSection.append("div")
        .attr("class", "country-info-item")
        .html(`
            <span class="country-info-label">Region:</span>
            <span class="country-info-value">${region}</span>
        `);
    ...
    factionSection.append("div")
        ...
        .html(`
            ...
            <span class="country-info-value">${currentEvents.length}</span>
        `);
    ...
    factionSection.append("div")
        ...
        .html(`
            ...
            <span class="country-info-value" style="color: #ef4444;">
                ${d3.format(",d")(casualties)}
            </span>
        `);
    ...
    factionSection.append("div")
        .attr("class", "country-info-item")
        .html(`
            <span class="country-info-label">Countries (${countries.length}):</span>
            <span class="country-info-value" style="font-size: 0.85rem;">
                ${countries.slice(0, 5).join(", ")}${countries.length > 5 ? ` +${countries.length - 5} more` : ''}
            </span>
        `);
    ...
    if (currentEvents.length > 0) {
        ...
        factionSection.append("div")
            ...
            .html(`
                <span class="country-info-label">Deadliest Event:</span>
                <span class="country-info-value" style="color: #ef4444;">
                    ${d3.format(",d")(deadliestEvent.best)} casualties
                </span>
            `);
...
// NOTE: Chart rendering functions (updateAllCharts, renderFactionTimelineChart, etc.)
// have been unified and moved to shared.js updateDashboardUI function.
// The main updateAllCharts function is defined later in this file (around line 2429).
...
function updateLeftPanel() {
    ...
    d3.select("#faction-filter-reset").remove();

    const region = data.region || 'Unknown';
    const currentCasualties = d3.sum(filteredEvents, e => e.best);

    // Create unified panel container (matching graph.js updateFactionPanel style)
    ...
    // Stats grid (2x2) - matching graph.js style
    ...
    eventsStat.append("div")
        ...
        .style("color", "#3b82f6")
        .text(d3.format(",d")(filteredEvents.length));
    ...
    casualtiesStat.append("div")
        .style("font-weight", "700")
        .style("color", "#ef4444")
        .text(d3.format(",d")(currentCasualties));

    // Connections stat (factions count)
    const factionCount = new Set();
    filteredEvents.forEach(e => {
        if (e.side_a) factionCount.add(e.side_a);
        if (e.side_b) factionCount.add(e.side_b);
    });
    ...
    // Activity by Year heatmap (title added by renderActivityHeatmap)
    if (filteredEvents.length > 0) {
        const heatmapContainer = countryPanel.append("div")
            .attr("id", "left-panel-heatmap")
            .style("background", "white")
            .style("border-radius", "6px")
            .style("padding", "1rem")
            .style("margin-bottom", "1rem")
        ...
        // Draw heatmap (function adds its own title)
        renderActivityHeatmap(heatmapContainer, filteredEvents);
    }
    ...
    // Connected Factions (Click to Filter)
    const factionCasualties = {};
    filteredEvents.forEach(event => {
        const sideA = event.side_a || '';
        const sideB = event.side_b || '';
        if (sideA) {
            if (!factionCasualties[sideA]) factionCasualties[sideA] = { casualties: 0, events: 0 };
            factionCasualties[sideA].casualties += event.deaths_a || 0;
            factionCasualties[sideA].events++;
        }
        if (sideB) {
            if (!factionCasualties[sideB]) factionCasualties[sideB] = { casualties: 0, events: 0 };
    ...
    const connectedFactions = Object.entries(factionCasualties)
        .map(([name, fData]) => ({ name, casualties: fData.casualties, events: fData.events }))
        ...
        connectedFactions.forEach(faction => {
            ...
            factionsContainer.append("div")
                ...
                .html(`
                    ...
                    <span style="color: #ef4444; font-weight: 600;">${d3.format(",d")(faction.casualties)}</span>
                `);
        });
    }

    // Deadliest Event - REMOVED per user request
